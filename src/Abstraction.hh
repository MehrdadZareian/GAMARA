...
public:
  ...
  void map_initialization(std::vector<state_type>& trajectory_states,scots::UniformGrid ss, Parameters parameters
      ...
      state_type r;
      state_type eta=parameters.s_eta;
      state_type m_z;
  ...
  void compute_gb3(TransitionFunction& transition_function,
                  F1& system_post,
                  F2& radius_post,
                  F3& avoid,//=params::avoid_abs,
                   ...
                   std::vector<abs_type>& LtoG,
                   abs_type* GtoL,
                   std::array<abs_type,9>* InputSet){ //__________________________1
    abs_type input_num=9; //@@@@@@@@@@@@@@@@@1
     /* number of cells */
    abs_type N=LtoG.size()+1; //__________________________2
    /* number of inputs */
    abs_type M=input_num; //@@@@@@@@@@@@@@@@@2
...
#pragma omp for schedule(dynamic, CHUNK_SIZE)
        for(abs_type i=0; i<N; i++) {
          ...
          for(abs_type j=0; j<M; j++) {
            ...
            for(int k=0; k<dim; k++)
              r[k]=eta[k]/2.0+m_z[k];
            /* current input */
            m_input_alphabet.itox(InputSet[i][j],u);
    ...
    //          double right = x[k]+r[k]+m_z[k];
            ...
            for(abs_type k=0; k<npost; k++) {  //_________NO CHANGE
              ...
              if(inside_of_area[q]==0 ){
                  transition_function.m_no_pre[(N-1)*M+j]++;
                  continue;
              }
...
//              }
  ...
  template<class F1, class F2>
  void compute_gb2_temp(TransitionFunction& transition_function,
                  F1& system_post,
                  F2& radius_post,
                   bool* inside_of_area,
                   std::vector<abs_type>& LtoG,
