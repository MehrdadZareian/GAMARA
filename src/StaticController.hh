...
public:
  ...
  std::vector<input_type> get_control_lazy(const state_type &x,
                                      abs_type* XTOI,
                                      bool* inside_area,
                                      std::array<abs_type,9>* InputSet) const {

      /* abstract state index */
    if(inside_area[m_state_grid.xtoi(x)]==0)
        std::cout<<"something is wrong"<<std::endl;
    abs_type x_i = XTOI[m_state_grid.xtoi(x)];
//      abs_type i = m_state_grid.xtoi(x);
    std::vector<abs_type> abs_inputs = m_winning_domain.get_inputs(x_i);
    //std::cout <<"here is reached"<<std::endl;

    if(!m_winning_domain.is_winning(x_i)) {
    ...
    for(abs_type i=0; i<abs_inputs.size(); i++) {
      input_type u;
      m_input_grid.itox(InputSet[x_i][abs_inputs[i]],u);
      inputs.push_back(u);
    }
    return inputs;
  }
