...
int main(){
    ...
    state_type initial_trajectory_state={{0,0,0}};
    ...
    for (int i = 0; i < state_dim; ++i) {
     double target_radius=parameters.epsilon_num[i]*parameters.s_eta[i];
        target_left[i]=nominal_trajectory_states[trajectory_size-1][i]-target_radius+parameters.s_eta[i]/2;
        target_right[i]=nominal_trajectory_states[trajectory_size-1][i]+target_radius-parameters.s_eta[i]/2;
    }
